...
import { convertFloorPlanTo3D, exportToOBJ } from "@/lib/floorplan/convert-to-3d";
...
export async function GET(
  ...
  try {
    ...
    const objBuffer = await exportToOBJ(scene);

    const safeTitle = access.project.title.replace(/\s+/g, "-").toLowerCase();
    const filename = safeTitle ? `${safeTitle}.obj` : `${id}.obj`;
    return new NextResponse(new Uint8Array(objBuffer), {
      headers: {
        "Content-Type": "application/octet-stream",
        "Content-Length": objBuffer.byteLength.toString(),
        "Content-Disposition": `attachment; filename="${filename}"`,
        "Cache-Control": "private, max-age=0, must-revalidate",
      },
    });
  } catch (err) {
    console.error("/api/projects/[id]/export/model-obj GET failed", err);
    return NextResponse.json({ error: "Failed to export OBJ model" }, { status: 500 });
  }
}